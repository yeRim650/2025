...
문자열을 저장하고 처리할 때 사용되는 주요 **자료구조와 알고리즘**은 다음과 같습니다. 각각의 구조와 알고리즘은 특정한 문제를 효과적으로 해결하기 위해 설계되었습니다.

---

## 🧱 문자열 저장 및 탐색에 주로 쓰이는 **자료구조**

### 1. **Trie (Prefix Tree, 트라이)**
- **목적**: 문자열의 집합에서 빠르게 검색, 삽입, 삭제를 하기 위해 사용
- **특징**:
  - 각 노드는 하나의 문자와 연결
  - 문자열의 접두사를 공유하여 저장 → 공간 효율적
- **복잡도**:
  - 검색/삽입/삭제: **O(L)** (`L`은 문자열 길이)
- **활용**:
  - 자동완성 기능
  - 사전(dictionary) 구현
  - 문자열 접두사 관련 문제

> 예: "apple", "app", "apply" → 'a' → 'p' → 'p'까지는 공통 경로를 공유
...
- **목적**: 문자열 내에서 **패턴 검색**을 빠르게 수행
- **핵심 아이디어**: 불일치 발생 시, 이전에 일치했던 정보를 활용해 **건너뛰기**
- **과정**:
  1. **LPS 배열 (Longest Prefix Suffix)**를 계산
  2. 검색 중 불일치가 발생하면, LPS 배열을 기반으로 다음 검사 위치를 결정
- **복잡도**: O(N + M) (`N` = 본문 길이, `M` = 패턴 길이)
- **활용**: 텍스트 내 패턴 찾기, DNA 서열 분석 등

---

### 3. **Rabin-Karp 알고리즘**
- **목적**: 문자열에서 **다수의 패턴 검색**을 빠르게 처리
- **핵심 아이디어**: **Rolling Hash** 사용 → 문자열의 해시값을 비교하여 빠르게 판단
- **특징**:
  - 해시 충돌이 발생할 수 있어 비교 과정 추가 필요
- **복잡도**:
  - 평균: O(N + M)
  - 최악: O(NM) (충돌이 자주 발생할 경우)
- **활용**:
  - 여러 패턴을 동시에 검색해야 할 때 유용
...
- **Suffix Array**:
  ...
  - 정렬된 상태로 이진 탐색 가능
- **Suffix Tree**:
  - 문자열의 모든 접미사를 압축 트리 형태로 저장
  - 서브스트링 검색에 매우 효율적
- **복잡도**:
  - 구축: O(N log N) 또는 O(N) (suffix tree)
  - 검색: O(M)
...
| 구조/알고리즘 | 목적                     | 시간복잡도       | 특징/용도                          |
|---------------|--------------------------|------------------|------------------------------------|
| Trie          | 빠른 문자열 저장/탐색   | O(L)             | 접두사 기반 자동완성 등            |
| KMP           | 패턴 검색                | O(N + M)         | 효율적인 실패 처리                 |
| Rabin-Karp    | 해시 기반 패턴 검색      | 평균 O(N + M)    | 다중 패턴 검색, 해시 충돌 주의     |
| Suffix Array  | 접미사 정렬              | O(N log N)       | 부분 문자열 탐색, 사전 순 정렬     |
| Suffix Tree   | 접미사 압축 트리         | O(N)             | 빠른 서브스트링 검색               |
